...
export async function syncServerTime() {
  try {
    ...
    if (res.ok) {
      ...
      serverTimeOffset = (data.timestamp + latency) - end;
    }
  } catch (e) {
...
// Start syncing immediately on load
      ...
      if (this.method === "insert") {
        ...
        for (const item of itemsToInsert) {
          ...
          if (!newItem.created_at) {
            newItem.created_at = getSyncedDate().toISOString();
          }
  ...
  async track(info: any) {
    ...
    try {
      ...
      await setDoc(presenceDocRef, sanitizeForFirestore({
        ...
        online_at: info?.online_at || getSyncedDate().toISOString(),
        last_ping: Date.now()
      }));
    } catch (e) {
    ...
    async signUp({ email, password, options }: any) {
      try {
        ...
        const profilePayload = {
          ...
          about_me: "",
          mood: "",
          last_online: getSyncedDate().toISOString(),
          language: "en",
          current_room: "main",
