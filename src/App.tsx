...
export default function App() {
  ...
  const fetchProfile = async (userId: string) => {
    ...
    if (data) {
      ...
      setUser({
        ...
        borderStyle: data.border_style || 'solid',
        cardBg: data.card_bg || undefined,
        cardGlowColor: data.card_glow_color || '#ff007f',
        cardGlowType: data.card_glow_type || 'none',
  ...
  const handleUpdateUser = async (updatedUser: Partial<UserProfile>) => {
    ...
    if ('borderStyle' in updatedUser) dbUpdate.border_style = updatedUser.borderStyle === undefined ? null : updatedUser.borderStyle;
    if ('profile_effect' in updatedUser) dbUpdate.profile_effect = updatedUser.profile_effect === undefined ? null : updatedUser.profile_effect;
    if ('cardBg' in updatedUser) dbUpdate.card_bg = updatedUser.cardBg === undefined ? null : updatedUser.cardBg;
    if ('cardGlowColor' in updatedUser) dbUpdate.card_glow_color = updatedUser.cardGlowColor;
    if ('cardGlowType' in updatedUser) dbUpdate.card_glow_type = updatedUser.cardGlowType;
