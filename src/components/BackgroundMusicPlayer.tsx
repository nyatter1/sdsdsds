...
export default function BackgroundMusicPlayer({ musicUrl, visualizerType = 'bars', soundsEnabled }: BackgroundMusicPlayerProps) {
  ...
  const hasUserPaused = useRef(false);
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      if (!hasUserPaused.current) {
        startPlayback();
      }
    }, 150);

    const handleUserInteraction = () => {
      if (audioRef.current && !isPlaying && !hasUserPaused.current) {
        startPlayback();
      }
