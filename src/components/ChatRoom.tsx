...
export default function ChatRoom({ user, onLogout, onUpdateUser }: ChatRoomProps) {
    ...
    if (data) {
      setAllProfiles(data.map(p => {
        ...
        return {
          ...
          borderStyle: p.border_style || "solid",
          profile_effect: p.profile_effect || "none",
          cardGlowColor: p.card_glow_color || "#ff007f",
          cardGlowType: p.card_glow_type || "none",
          is_muted: p.is_muted,
  ...
  const mapDbProfileToOnlineUser = (p: any): OnlineUser => {
    ...
    return {
      ...
      username_format: p.username_format || "normal",
      message_color: p.message_color || "#e9d5ff",
      message_font: p.message_font || "Inter",
      message_effect: p.message_effect || "none",
      message_format: p.message_format || "normal",
      isSystem: false,
  ...
  const getUserCardStyle = (u: any) => {
    let style: React.CSSProperties = {};
    let className = "";

    // 1. Glow Types
    if (u.cardGlowType === 'terminal') {
      style = {
        backgroundColor: '#000000',
        borderColor: '#00ff00',
        boxShadow: '0 0 12px rgba(0, 255, 0, 0.6)',
      };
      className = 'font-mono text-[#00ff00] border-solid border border-[#00ff00]';
    } else if (u.cardGlowType === 'rainbow') {
      className = 'animate-rainbow-card border';
    } else if (u.cardGlowType === 'color' && u.cardGlowColor) {
      style = {
        borderColor: u.cardGlowColor,
        boxShadow: `0 0 10px ${u.cardGlowColor}`,
      };
    ...
    if (u.cardGlowType !== 'terminal' && u.border && u.border !== 'none') {
      ...
      if (u.borderStyle && u.borderStyle !== 'solid') {
        ...
        if (payload.eventType === 'INSERT' && payload.new) {
          ...
          setOnlineUsers(prev => {
            const updatedList = prev.map(u => 
              u.id === payload.new.id ? { 
                ...
                borderStyle: payload.new.border_style !== undefined ? payload.new.border_style : u.borderStyle,
                profile_effect: payload.new.profile_effect !== undefined ? payload.new.profile_effect : u.profile_effect,
                cardGlowColor: payload.new.card_glow_color !== undefined ? payload.new.card_glow_color : u.cardGlowColor,
                cardGlowType: payload.new.card_glow_type !== undefined ? payload.new.card_glow_type : u.cardGlowType,
          ...
          if (user && payload.new.id === user.id) {
            onUpdateUserRef.current({
              ...
              borderStyle: payload.new.border_style !== undefined ? payload.new.border_style : user.borderStyle,
              profile_effect: payload.new.profile_effect !== undefined ? payload.new.profile_effect : user.profile_effect,
              cardGlowColor: payload.new.card_glow_color !== undefined ? payload.new.card_glow_color : user.cardGlowColor,
              cardGlowType: payload.new.card_glow_type !== undefined ? payload.new.card_glow_type : user.cardGlowType,
          ...
          setProfileTarget(prev => {
            if (prev && prev.id === payload.new.id) {
              return {
                ...
                borderStyle: payload.new.border_style !== undefined ? payload.new.border_style : prev.borderStyle,
                profile_effect: payload.new.profile_effect !== undefined ? payload.new.profile_effect : prev.profile_effect,
                cardGlowColor: payload.new.card_glow_color !== undefined ? payload.new.card_glow_color : prev.cardGlowColor,
                cardGlowType: payload.new.card_glow_type !== undefined ? payload.new.card_glow_type : prev.cardGlowType,
                          ...
                          >
                            <div className="flex items-center gap-2.5 min-w-0">
                              ...
                              <div className="min-w-0">
                                <span 
                                  className={`text-xs font-bold truncate max-w-[120px] flex items-center gap-1 ${u.cardGlowType === 'terminal' ? 'text-[#00ff00]' : 'text-white'} ${getStyleClasses(u.username_effect, u.username_format)}`}
                                  style={u.cardGlowType === 'terminal' ? { color: '#00ff00', fontFamily: 'monospace' } : getStyleInline(u.username_color, u.username_font, u.username_effect)}
                                >
                                  {u.username}
                                  {u.is_muted && <Hand className="w-3 h-3 text-red-500" title="Muted" />}
                                </span>
                                <div className="flex items-center gap-2 mt-0.5">
                                  <span className="text-[9px] italic truncate max-w-[120px] block" style={{ color: u.cardGlowType === 'terminal' ? '#00ff00' : '#a78bfa' }}>{u.mood || "No mood set"}</span>
                                </div>
                              </div>
                            </div>
                        ...
                        return (
                          ...
                          >
                            <div className="flex items-center gap-2.5 min-w-0">
                              ...
                              <div className="min-w-0">
                                <span 
                                  className={`text-xs font-bold truncate max-w-[120px] flex items-center gap-1 ${u.cardGlowType === 'terminal' ? 'text-[#00ff00]' : 'text-purple-200'} ${getStyleClasses(u.username_effect, u.username_format)}`}
                                  style={u.cardGlowType === 'terminal' ? { color: '#00ff00', fontFamily: 'monospace' } : getStyleInline(u.username_color, u.username_font, u.username_effect)}
                                >
                                  {u.username}
                                  {u.is_muted && <Hand className="w-3 h-3 text-red-500" title="Muted" />}
                          ...
                          return (
                            ...
                            >
                              <div className="flex items-center gap-2 min-w-0">
                                ...
                                <div className="min-w-0">
                                  <span 
                                    className={`text-[11px] font-bold truncate max-w-[120px] flex items-center gap-1 ${u.cardGlowType === 'terminal' ? 'text-[#00ff00]' : isOnline ? "text-white" : "text-purple-300"} ${getStyleClasses(u.username_effect, u.username_format)}`}
                                    style={u.cardGlowType === 'terminal' ? { color: '#00ff00', fontFamily: 'monospace' } : getStyleInline(u.username_color, u.username_font, u.username_effect)}
                                  >
                                    {u.username}
                                    {u.is_muted && <Hand className="w-3 h-3 text-red-500" title="Muted" />}
                                  ...
                                  {u.mood && (
                                    <span className="text-[8px] italic truncate max-w-[120px] block" style={{ color: u.cardGlowType === 'terminal' ? '#00ff00' : '#a78bfa' }}>
                                      {u.mood}
                                    </span>
                                  )}
                                </div>
      ...
      {profileTarget && (
        <ProfileModal
          ...
          onUpdate={async (updated: any) => {
            ...
            if ('cardGlowColor' in updated) mapped.card_glow_color = updated.cardGlowColor;
            if ('cardGlowType' in updated) mapped.card_glow_type = updated.cardGlowType;
